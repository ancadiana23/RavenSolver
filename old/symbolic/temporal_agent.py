...
def run_solver_BKTM(problems, param):
    ...
    tm = BTM(numberOfCols=SDRlen, cellsPerColumn=20,
             initialPerm=0.5, connectedPerm=0.5,
             minThreshold=4, newSynapseCount=15,
             activationThreshold=6,
             pamLength=10)

    for _ in range(epochs):
        for problem in np.random.permutation(problems):
            correct_answer = problem['result'] - 1
            ...
            tm.compute(problem['SDRs']['output'][correct_answer], enableLearn=True, enableInference=True)
            tm.reset()
    ...
    for j in range(len(problems)):
        ...
        tm.compute(problem['SDRs']['input'][0], enableLearn=False, enableInference=True)
        tm.compute(problem['SDRs']['input'][1], enableLearn=False, enableInference=True)
        tm.compute(problem['SDRs']['input'][2], enableLearn=False, enableInference=True)
        ...
        max_match = max(match)
        #print(max_match)
        vote = match.index(max_match)
        correct_answer = problem['result'] - 1

        if vote == correct_answer:
            correct_predictions[j] = max_match
        results.append(vote)
        '''
        print(np.sum(predictedCells),
              np.sum(problem['SDRs']['output'], axis=1),
              np.sum(problem['SDRs']['input'], axis=1))
        print(problem['title'], correct_answer, vote, match[correct_answer], match[vote])
        '''
        ...
        for i in range(len(best_param)):
            ...
            for sign in [1, -1]:
                ...
                if  x >= limits[i][0] and x <= limits[i][1]:
                    new_param = best_param[:i] + \
                                ((best_param[i] + sign * steps[i]),) + best_param[i + 1:]
        ...
        for (x, y) in [('A', 'input'), ('1', 'output')]:
            ...
            while chr(index) in problem['content']:
                ...
                for x in new_SDR:
                    for _ in range(20):
                        SDR += [x]
                problem['SDRs'][y].append(np.array(SDR))
                index += 1
